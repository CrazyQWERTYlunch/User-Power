...
async def delete_user(user_id: UUID, session: AsyncSession = Depends(get_async_session)) -> DeleteUserResponse:
...
async def get_user_by_id(user_id: UUID, session: AsyncSession = Depends(get_async_session)) -> ShowUser:
...
async def update_user_by_id(
    ...
    if user is None:
        raise HTTPException(status_code=404, detail=f"User with id {user_id} not found")
    updated_user_id = await _update_user(updated_user_params=updated_users_params, user_id=user_id, session=session)
